...
class Trainer():
    ...
    def post_train_rotate(self, iters, model_copy, 
                          lr=0.001, task_iters=None, loss_lambda=None, 
                          decay_lr_freq=4000, decay_lr_rate=0.5, 
                          writerPath=None, savePath=None, reload=None,
                          ext=""):
        self.model.train()
        ...
        if reload is not None and savePath is not None:
            ...
            if 'post_train' in reload:
                start = state['iter'] + 1
                self.model.load_state_dict(state['state_dict'])
                model_copy.load_state_dict(state['state_dict'])
            else:
                # Only load the policy parmas
                policy_params = state['state_dict'].copy()
                for key in state['state_dict']:
                ...
                print(policy_params, flush=True)
                self.model.load_state_dict(policy_params, strict=False)
                model_copy.load_state_dict(policy_params, strict=False)

        sd = self.model.state_dict()        # the model used for training
        sd_copy = model_copy.state_dict()   # the model used to copy weight, not trained
        ...
        if reload is not None and savePath is not None:
            if 'post_train' in reload:
                optimizer.load_state_dict(state['optimizer'])
        scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=decay_lr_freq, gamma=decay_lr_rate)
        # scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=iters, eta_min=4e-08)

        for i in range(start, iters):
            sd_copy = model_copy.state_dict()
            ### rotate and train
            # sd["diffractive_layers.models.1.taskOp.mnist.phase"] = torch.rot90(sd_copy["diffractive_layers.models.1.taskOp.fmnist.phase"], 3, [0, 1])
            sd["diffractive_layers.models.1.taskOp.mnist.phase"] = torch.rot90(sd_copy["diffractive_layers.models.1.taskOp.fmnist.phase"], 2, [0, 1])
            sd["diffractive_layers.models.1.taskOp.kmnist.phase"] = torch.rot90(sd_copy["diffractive_layers.models.1.taskOp.fmnist.phase"], 1, [0, 1])
            # sd["diffractive_layers.models.1.taskOp.kmnist.phase"] = torch.rot90(sd_copy["diffractive_layers.models.1.taskOp.fmnist.phase"], 2, [0, 1])
            ...
            ### rotate back
            # sd["diffractive_layers.models.1.taskOp.mnist.phase"] = torch.rot90(sd["diffractive_layers.models.1.taskOp.mnist.phase"], 1, [0, 1])
            sd["diffractive_layers.models.1.taskOp.mnist.phase"] = torch.rot90(sd["diffractive_layers.models.1.taskOp.mnist.phase"], 2, [0, 1])
            sd["diffractive_layers.models.1.taskOp.kmnist.phase"] = torch.rot90(sd["diffractive_layers.models.1.taskOp.kmnist.phase"], 3, [0, 1])
            # sd["diffractive_layers.models.1.taskOp.kmnist.phase"] = torch.rot90(sd["diffractive_layers.models.1.taskOp.kmnist.phase"], 2, [0, 1])
            ### average the weight
            tmp_weght = (sd["diffractive_layers.models.1.taskOp.mnist.phase"] + \
                            sd["diffractive_layers.models.1.taskOp.kmnist.phase"] + \
                                sd["diffractive_layers.models.1.taskOp.fmnist.phase"] + \
                                    sd["diffractive_layers.models.1.taskOp.emnist.phase"]) / 4
            for param_tensor in sd_copy: 
                if ("models.1.taskOp" in param_tensor) and ("phase" in param_tensor):
                    sd_copy[param_tensor] = tmp_weght
            
            ### load the weight
            model_copy.load_state_dict(sd_copy)
            ...
            if (i+1) % self.save_iters == 0:
                if savePath is not None:
                    state = {'iter': i,
                             'state_dict': self.model.state_dict(),
                             'optimizer': optimizer.state_dict()}
                    self.save_model(state, f'post_train_rotate_Mdlavg{ext}', savePath)
    ...
    def train_step(self, stage, optimizer, scheduler=None, alter_train=None, tau=1, hard=False, policy_idx=None, loss_lambda=None):
        ...
        for task in self.tasks: 
            ...
            x, y = utils.data_to_cplex(data, device=self.device)
